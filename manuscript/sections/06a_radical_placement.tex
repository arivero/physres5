...
\toprule
Placement & Physical meaning & Derivation owed\\
\midrule
...
This list is useful because it prevents the radical from floating between schemes.  The manuscript can change placement; each placement changes the required derivation.

\subsection{Placement ledger}
\label{sec:placement-ledger}

The placement question can be recorded as a ledger.  A source theory \(\mathcal S\) supplies an operator, the operator supplies a two-channel kernel, the kernel supplies branches, and the branches acquire physical meaning after a scheme and an assignment rule have been fixed:
\begin{equation}
\begin{aligned}
\mathcal S
&\longrightarrow
...
\right\}.
\end{aligned}
\label{eq:placement-ledger}
\end{equation}
Here \(\mathcal F_{\rm sc}\) denotes the scalar/order-parameter functional sought in Sec.~\ref{sec:negative}, and \(\mathcal C_{\rm top}\) denotes the global-form and representation constraints discussed in Secs.~\ref{sec:flavour} and \ref{sec:global-form}.  The ledger separates four tasks:
\begin{align}
  \text{operator origin:}\quad&
  \mathcal S\mapsto \mathcal O_J,
  \label{eq:ledger-origin}\\
  \text{two-channel reduction:}\quad&
  \mathcal O_J\mapsto
  \begin{pmatrix}0&\sqrt J\\ \sqrt J&-J\end{pmatrix},
  \label{eq:ledger-reduction}\\
  \text{branch reading:}\quad&
  \mathcal M_J\mapsto \{x_+(J),x_-(J)\},
  \label{eq:ledger-branch}\\
  \text{physical assignment:}\quad&
  \{x_\pm\}\mapsto
  \left(\sPole,\mathcal F_{\rm sc},\mathcal C_{\rm top}\right).
  \label{eq:ledger-assignment}
\end{align}

Each arrow has different source support.  Complex-pole and scheme language belongs to the last arrow \cite{MartinRobertson2025}.  Higgs doublet, adjoint current, custodial structure, and the electroweak ray belong to the assignment arrow \cite{Logan2022,Dawson2018}.  Endpoint labels, D-brane gauge fields, and interval boundary conditions belong to the first two arrows \cite{TongString,Polchinski1994,CsakiHubiszMeade2005}.  The current manuscript has exact control of Eq.~\eqref{eq:ledger-branch}; the other arrows are open derivation targets.

This ledger also organizes the negative branch.  If a candidate source theory derives \(\mathcal M_J\), then \(x_-(J)\) is carried by the same light-sector basis as \(x_+(J)\).  The scalar question becomes a question about the eigenvector map,
\begin{equation}
  u_-(J)\in\mathcal H_J
  \quad\longmapsto\quad
  \mathcal F_{\rm sc},
  \label{eq:negative-eigenvector-ledger}
\end{equation}
...
The ledger gives a compact criterion for progress.  A new idea improves the paper when it fills one of the arrows in Eq.~\eqref{eq:placement-ledger}; it distracts from the paper when it adds labels outside the ledger.
...
\paragraph*{Section status.}  This section states the radical-placement problem and the original research route.  The physical operator \(\mathcal O_J\) remains open.

...
\end{equation}
The entry priority is now fixed.  The interval/CHM route supplies the first
pass/fail theorem through the hatted current entry
\(\widehat\Sigma_{aa,J}^{\rm CHM}=J\).  The Hodge/SUSY-QM route supplies the
companion square-root theorem
\(\Sigma_{ha,J}\Sigma_{ah,J}=J\).  The \(G_2\) route supplies a local-kernel
audit.  The endpoint/Chan--Paton route supplies a matrix arena whose DeVries
use requires a worldvolume quadratic kernel, projection, and normalization.
...
The interval arena supplies source-derived boundary data.  The rendered CHM
audit fixes the boundary kinetic normalization, the corresponding modified
...
records the detailed convention ledger.  In compact route notation,
\begin{equation}
...
source-controlled derivation.  The entry tests now have two linked forms:
Hodge/SUSY-QM factorization tests
\(\Sigma_{ha,J}\Sigma_{ah,J}=J\), while the CHM boundary current tests
\(\Sigma_{aa,J}=J\).  The immediate normalization task is the current-channel
entry in dimensionless variables.  Let
\(\widehat\lambda=\lambda/\Lambda_{\rm CHM}^2\).  Let \(P_\gamma\) select the
photon reference trace, and set
\[
  P_\gamma^\dagger
  \left(K_{T,J}(0)-K^{\rm ref}_{T,J}\right)
  P_\gamma
  =
  0,
  \qquad
...
\end{aligned}
  \label{eq:chm-current-entry-target}
\end{equation}
where \(a_J=P_{a,J}A_\mu^{T,\partial}\) is a transverse boundary-current
sample.  The entry theorem is
\begin{equation}
  \widehat K^{\rm cur}_J(\widehat\lambda)
  =
  ...
  \label{eq:chm-current-entry-normalization}
\end{equation}
The CHM source controls the boundary variation, product, vector Robin data,
scalar \(A_5/\pi_i\) equations, custodial bookkeeping, and photon-zero
accounting.  The projection \(P_{a,J}\), reference subtraction, source scale
\(\Lambda_{\rm CHM}\), endpoint-normal conventions, ordered W/Z boundary map,
and pole map remain theorem data.  The parent KK-fixing notes sharpen the same issue into a single-source
test: the dimensional middle line, negative-branch scalar reading, and
electromagnetic endpoint can be combined only after one source variable
supplies
\begin{equation}
  u\mapsto
...
\end{equation}
% status: open; the electromagnetic endpoint route is a downstream theorem
% target tied to the same source datum as the vector and scalar routes.
Thus the scalar and electromagnetic readings sit downstream of the current
entry, ordered W/Z assignment, source-to-pole calculation, and photon-zero
normalization.  The O17 endpoint route requires the same source datum \(u\)
to provide \(P_\gamma\), \(P_W\), \(P_Z\), \(\Lambda_J\),
\(\mathcal F_{\rm sc}\), and a compact generator
\(\xi_{\rm em}\).  Its pass/fail chain is
\[
u\to
...
\]
This places the alpha route behind the CHM current-entry theorem,
the ordered sampling theorem, and the negative-branch scalar theorem.
...
defines their common inner product.  The Wigner--Eckart route is a recorded
failed tested subroute, so the superconnection route must supply its own
projection.  The SO(32) material remains flavor-boundary bookkeeping until a
...
\midrule
\(\Sigma_{aa}=J\) & Chan--Paton current or brane gauge amplitude & CHM boundary current-kernel target with rendered source conventions; \(J\)-normalization open & singular gauge locus, Higgs-bundle one-form, or adjoint local operator with compactification-scale matching\\

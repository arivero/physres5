...
\end{equation}
The diagonal entries describe strings whose endpoints lie on the same brane.  The off-diagonal entries describe strings stretched between distinct branes.  At coincidence the off-diagonal vector modes fill out the nonabelian gauge multiplet.  This is precisely the type of two-channel structure needed by Eq.~\eqref{eq:minimal-two-channel}.
...
\end{equation}
Endpoint data therefore supply the matrix arena.  The equalities
\(\kappa_J^2=\tau_J=J\), the projection to \(\mathcal H_J\), and the branch
survival rule remain source-kernel theorem data.
...
\toprule
DeVries entry & Endpoint or brane origin & Source condition \\
\midrule
...
\subsection{Endpoint route criterion}

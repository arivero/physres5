...
Hsin and collaborators propose a transport-style probe of this global form through a fractional topological response coefficient \cite{HsinGomis2024}.  The important point here is structural: the global form is, in principle, observable through nonlocal or topological response data.  It supplies a completion constraint for the DeVries program, because a string or compactification model generally determines more than a local Lie algebra.

Grand unification provides the comparison class.  SU(5) and Spin(10) naturally package Standard Model representations in a way that favors the \(\mathbb Z_6\) quotient, and the subgroup \(S(U(2)\times U(3))\) realizes the quotient structure inside SU(5) \cite{BaezHuerta2010,Britto2021}.  This places the GUT lineage beside the DeVries pole reading: high-scale structural placement is already known in unified models, while the DeVries construction assigns its algebraic value to the low-energy pole ratio and therefore needs a separate pole-placement derivation.
...
\end{equation}
where the upper arrow fixes \(\Gamma\) and the lower arrow reduces the ultraviolet spectral operator to the DeVries two-channel operator.  A model that supplies \(\mathcal O_J\) while leaving \(\Gamma\) unspecified has an underdetermined topological sector.

...
source-controlled derivation.  The immediate target is the current-channel
entry
\begin{equation}
  K^{\rm cur}_J(\lambda)
  =
...
accounting.  The projection \(P_{a,J}\), reference subtraction, dimensionless
\(J\)-normalization, ordered W/Z boundary map, and pole map remain theorem
data.  The parent KK-fixing notes sharpen the same issue into a single-source

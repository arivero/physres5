...
\end{equation}
The source-equation audit for this target is explicit:
\begin{equation}
  \Phi=\sum_i t_i df_i,\qquad
  \rho=\sum_i t_i\rho_i,\qquad
  \Delta f_i=\rho_i,
  \label{eq:g2-source-equation-audit-one}
\end{equation}
and
\begin{equation}
  ...
  df_Q(p)=0.
  \label{eq:g2-source-equation-audit-two}
\end{equation}
The current entry \(\Sigma_{aa,J}^{G_2}=J\) and the flow-overlap product
\(\Sigma_{ha,J}^{G_2}\Sigma_{ah,J}^{G_2}=J\) are local entry targets requiring
the operator \(\mathcal O_J^{G_2}\) and the pairing on
\(\mathcal H_J^{G_2}\).
The operator, pairing, and equality to \(J\) are theorem data for the \(G_2\)
route.
...
\(\Sigma_{aa,J}^{G_2}=J\).
At this stage the \(G_2\) route specifies local operators through
\(\mathcal H_J^{G_2}\) and its pairing.  A CHM-comparable current entry requires
four additional data: the pairing, compact completion, charge lattice, and pole
map.

...
The SO(32)-flavor idea is therefore an organizing boundary sector with
project-source status in the present manuscript.  In an endpoint language it may
label external or internal flavor degrees of freedom, Chan--Paton-like data, or
a bookkeeping algebra for fermion multiplicities.  The present representation
audit records the proposed branching data
\begin{equation}
  16=(5,3)+(1,1),
  \qquad
  \wedge^2(5,3)=(15,\bar 3)+(\overline{10},6).
  \label{eq:so32-flavor-branching-audit}
\end{equation}
These equations currently function as bookkeeping obligations.  Journal-facing
use requires a primary source or a derived embedding in which the endpoint or
...
\item project, lift, or decouple extra states from Eq.~\eqref{eq:so32-flavor-branching-audit};

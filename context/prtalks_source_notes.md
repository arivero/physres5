...
| `../prTalks/Regge scaling of mass with spin in superstrings - Claude.pdf` | source note with visual plots | The positive DeVries branch is bounded for fixed input scale, while the negative branch has magnitude growing like \(J\) in mass and like \(J^2\) in mass squared. The plots compare point, string, brane, positive-root, and negative-root scaling. The exact product \(x_+(J)x_-(J)=-J\) gives a geometric-mean Regge invariant \(\sqrt{-M_+^2M_-^2}\propto\sqrt{J}\) in the normalized variable, with a second option where \(j\) labels an intercept and a new oscillator \(n\) supplies ordinary linear Regge growth. |

...
Eq.~\eqref{eq:interval-self-energy-target} is the open DeVries entry theorem.
...
\end{equation}
where the denominators are positive and
\[
  \langle a_J^{\gamma^\perp},a_\gamma\rangle_{\rm CHM}=0.
\]
The source theory must state the ordered product
\(P_{a,J}\Pi_{\gamma^\perp}^{(T)}\) or prove that the two projectors commute on
\(\mathcal N_T^\partial\).  In the current-entry formulas below \(a_J\)
abbreviates \(a_J^{\gamma^\perp}\).  The transverse kernel is
\[
  K_{T,J}=K^{\rm DtN}_{T,J}+K^{\rm brane}_{T,J}.
\]
A photon-reference subtraction fixes the zero by
\begin{equation}
  P_\gamma^\dagger
  ...
  =
  0.
  \label{eq:chm-photon-reference-subtraction}
\end{equation}
Let \(\Lambda_{\rm CHM}\) denote the shared source scale that makes the
spectral variable dimensionless.  Equivalently, after a \(J\)-sample has been
chosen, this is the route's \(\Lambda_J\):
\begin{equation}
...
\end{equation}
where \(K_{\gamma,J}^{\rm ref}\) fixes the reference zero of the current kernel,
through the unbroken photon channel or a stated boundary subtraction.  The
constant entry in this normalization is equivalently
\begin{equation}
...
\(\Lambda_{\rm CHM}\), endpoint signs, the CHM product, and the pole map in a
single convention.
...
\begin{equation}
  ...
  \qquad
  \widehat\Sigma_{aa,J_{\rm adj}}^{\rm CHM}=J_{\rm adj}=2,

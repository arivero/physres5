...
- Expanded the electroweak embedding around custodial structure and the assignment theorem target.
- Added a toy endpoint-boundary operator with named two-channel fields.
- Added theorem-target appendix for pole placement, electroweak assignment, boundary determinant, scalar branch, and global-form compatibility.
- Added endpoint gauge-data and brane-Higgsing section with a source-backed dictionary for the two-channel operator.
...
- Page-check the fragments cited in `context/concept_claims_matrix.md`.
- Add missing primary/historical sources for:
  - Georgi-Quinn-Weinberg weak-angle derivation;
  - early dual resonance/string phenomenology;
  - QCD string/Regge slope context;

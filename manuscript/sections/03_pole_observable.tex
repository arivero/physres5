...
\label{sec:pole}
% Status: cited/derived. Observable, placement convention, and current descriptive input audit are stated.
...
The pole reading selects the first row.  The remaining rows are translation targets.  The current source-audited electroweak comparison begins from the quoted experimental convention, maps to complex pole quantities, forms Eq.~\eqref{eq:spole}, and then compares with Eq.~\eqref{eq:sdvdef}.
...
\subsection{Current W/Z input audit}
\label{sec:current-wz-audit}
...
\begin{equation}
  \left(s^2_{\rm pole}\right)_{\rm PDG\,audit}
  =
  1-\frac{(80.3407\,{\rm GeV})^2}{(91.1538\,{\rm GeV})^2}
  =
  0.2231768\pm0.0002608 .
  \label{eq:spole-current-audit}
\end{equation}
This is the current W/Z input line used for descriptive comparison in this
manuscript.  The CDF-II measurement and the CMS 2026 measurement
...
for Eq.~\eqref{eq:spole-current-audit} without a common Z input, width

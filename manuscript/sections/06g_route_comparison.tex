...
\(G_2\) route supplies a local-kernel test.  The endpoint/Chan--Paton route
...
The route is fully specified when one source package supplies the projected
trace space, source product, photon reference, scalar functional, and pole map,
and derives the arrows in
\begin{equation}
\begin{array}{ccc}
...
\end{equation}
The three proof obligations are the canonical CHM neutral-current entry
\(\widehat\Sigma_{aa,2}^{\rm CHM,can}=2\), the same-basis Hodge/SUSY-QM
...
The interval route is the current test arena because the CHM boundary conditions

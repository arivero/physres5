...
\end{equation}
The available source equations fix the candidate entry order.  The
interval/CHM route supplies the first current-entry theorem target through
\(\widehat\Sigma_{aa,J}^{\rm CHM}=J\).  The Hodge/SUSY-QM route supplies the
companion square-root theorem target
\(\Sigma_{ha,J}\Sigma_{ah,J}=J\).  The \(G_2\) route supplies a local-kernel
test.  The endpoint/Chan--Paton route supplies a matrix arena whose DeVries
...
The route closes when one source package supplies the projected trace space,
source product, photon reference, scalar functional, and pole map, and derives
every arrow in
\begin{equation}
\begin{array}{ccc}
...
\end{equation}
The three proof obligations are the CHM neutral-current entry
\(\widehat\Sigma_{aa,2}^{\rm CHM}=2\), the same-basis Hodge/SUSY-QM
off-diagonal product \(\Sigma_{ha,J}\Sigma_{ah,J}=J\), and the ordered
projector package \(P_W,P_Z,P_\gamma\) with its pole remainder.
The proof order is fixed by the CHM current entry, the
\(\Delta_W^{\rm CHM}/\Delta_Y^{\rm CHM}\) projector package, the same-basis
Hodge product, and the pole-scheme match to \(\Delta_{\rm O1/O10}\).
...
records the convention ledger.  In compact route notation,
\begin{equation}
...
The interval route is the active test case because the CHM boundary conditions
provide explicit vector and scalar boundary data for an entry-level derivation
from the source equations.  The entry tests have two linked forms:
Hodge/SUSY-QM factorization tests
\(\Sigma_{ha,J}\Sigma_{ah,J}=J\), and the CHM boundary current tests
\(\Sigma_{aa,J}=J\).  The current-channel subproblem is the entry in
dimensionless variables.  Let
...
\end{equation}
where \(a_J=P_{a,J}^{\gamma^\perp}A_\mu^{T,\partial}\) is a transverse
photon-orthogonal boundary-current sample in the neutral CHM product.  The
entry theorem is
\begin{equation}
  \widehat K^{\rm cur}_J(\widehat\lambda)
  =
...
\end{equation}
The CHM equations supply the boundary variation, product, vector Robin data,
...
\midrule
\(\Sigma_{aa}=J\) & Chan--Paton current or brane gauge amplitude & CHM boundary current-kernel target with source conventions; \(J\)-normalization open & singular gauge locus, Higgs-bundle one-form, or adjoint local operator with compactification-scale matching\\

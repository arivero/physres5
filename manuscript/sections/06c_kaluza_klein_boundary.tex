...
This gives the manuscript a useful analogy:
\begin{equation}
  \left(
  \begin{array}{c}
...
\end{equation}
Here \(C_2\) denotes a quadratic invariant, \(h_{\rm CFT}\) a conformal weight or current-algebra datum, \(q_{\rm KK}\) a compact charge, and the overlap term a normalized boundary/bulk integral.  The final paper should name one of these entries and derive the equality to \(J\).
...
\end{equation}
where \(v\) is radial and \(\mathcal P(g,g')\) is projective.  The DeVries ratio can act on \(\mathcal P(g,g')\), or on a boundary operator whose pole condition reduces to that same projective datum.  Thus the interval model should yield a coupled charged/neutral construction:
\begin{equation}
  \left[
  \begin{array}{c}
...
\end{equation}
Here \(u_-(J)\) is the negative-branch eigenvector of the two-channel kernel.  A final derivation should identify \(\mathcal F_{\rm KK}\), state its gauge transformation properties, and explain its scheme relation to the low-energy Higgs/order-parameter variables.

\subsection{Hard test for the KK route}
\label{sec:kk-hard-test}

The KK route becomes a hard test once it is expressed as the following chain:
\begin{equation}
\begin{aligned}
...
\paragraph*{Section status.}  The KK circle, interval, and boundary-condition background is source-supported.  The reductions in Eqs.~\eqref{eq:interval-self-energy-target}, \eqref{eq:interval-assignment-theorem}, and \eqref{eq:kk-negative-branch-map} are theorem targets.

...
\(SU(3)\times U(1)\) symmetry address & derive the infinite-Higgs or
infinitely broken endpoint with \(U(1)_{\rm em}\) as the surviving
electroweak factor\\
\bottomrule
\end{tabular}
\end{center}
The middle line is therefore the active theorem target.  A derivation must
identify the six-dimensional internal object, its boundary or compact
operator, and its relation to the two-channel DeVries determinant.
...
\end{equation}
This version makes the six-dimensional middle line especially natural as a
string-theory address.  Six-dimensional superstring vacua and dualities are a
...
existence and rigidity of six-dimensional string settings.  The DeVries kernel,
ordered \(J\)-assignment, and pole placement remain separate reduction targets.

\subsection{Interpolation parameter}
\label{app:interpolation-parameter}

The interpolation parameter must be stated in field-theory or geometric terms.
The electroweak field theory supplies a radial order parameter through the Higgs
vacuum expectation value.  The compactification descriptions supply radii,
boundary conditions, brane-localized terms, or singularity deformations.  A
viable derivation must identify one variable \(t\) and a family of source
operators
\begin{equation}
  \mathcal O_J(t),
  \qquad
  t=0:\ SU(3)\times SU(2)\times U(1),
  \qquad
  t=\infty:\ SU(3)\times U(1)_{\rm em},
  \label{eq:dimensional-interpolation-family}
\end{equation}
or the corresponding colourless electroweak endpoints.  The middle-dimensional
...
\end{equation}
where \(\mathcal B(t_\star)\) is the compact, boundary, or singular object,
\(\mathcal H_J(t_\star)=\operatorname{span}\{h_J,a_J\}\) is the light
...
&:\quad
\mathcal O_J(t_\star)\big|_{\mathcal H_J}\to K_J(\lambda),
\\
K_J(\lambda)
&\longrightarrow
...
kernel derivation, an ordered assignment theorem, and a pole-matching theorem.

\section{Two-channel secular mass-ratio operator}
\label{sec:casimir}
% Status: derived/open. Eigenvalue algebra is derived; uniqueness remains an analytical subtask.

Let \(J=s(s+1)\) be the historical quadratic-invariant input.  The normalized two-channel operator used in this paper is
\begin{equation}
...
\end{equation}
Thus a physical derivation has to explain a simultaneous trace and determinant assignment.  A model that gives only a square-root branch, or only a \(J\)-dependent diagonal term, leaves the DeVries construction underdetermined.  The required two-channel matrix has the schematic form
\begin{equation}
  \mathcal M_J=
  \begin{pmatrix}
  0 & \kappa_J\\
...
Trace and determinant statements in this paper are matrix-specific.  The trace
of \(Q(J)\), the trace of any first-seed kernel \(K_{\rm seed}^{(1)}\), and the
trace of any \(\sigma_3\)-projected negative-sector kernel \(K_-^{\sigma_3}\)
belong to distinct vector spaces until a common source operator, field basis,
projection map, and normalization identify them.  A coefficient transfer
between these matrices requires an explicit source-theory theorem.
...
\subsection{Minimal assumptions and source test}

A source derivation must state the minimal assumptions that select Eq.~\eqref{eq:QJ}.  The current minimal list is:
\begin{enumerate}
...
\end{enumerate}
These assumptions define the uniqueness test.  The missing calculation is: within a stated class of two-channel operators built from the allowed invariants, the high-spin condition and trace/determinant constraints select Eq.~\eqref{eq:QJ} up to similarity transformations and normalization.

...
in this manuscript is local.  It gives a framework in which gauge fields,
...
\begin{equation}
  ...
  \quad\hbox{inside}\quad
  \mathcal H_J^{G_2}=\operatorname{span}\{h_J^{G_2},a_J^{G_2}\}.
  \label{eq:g2-mw-to-ha-test}
\end{equation}
Loop 49 records the failed derivation step: the Braun source data lack an
identification of the projected ADE current \(a_J^{G_2}\) as a
Morse--Witten endpoint state in this two-channel block.  Target IIIh therefore
remains active only after an additional same-basis projection theorem supplies
...
construction from the local \(G_2\) gauge theory is part of the theorem data.
...
\midrule
\(\Sigma_{aa,J}^{G_2}=J\) & ADE singular gauge sector with \(A_1\subset A_2\) enhancement and singular-support pairing & derive the canonical current entry \(\widehat\Sigma_{aa,2}^{G_2,{\rm can}}=2\) \\
\(\Sigma_{ha,J}^{G_2}\Sigma_{ah,J}^{G_2}=J\) & localized matter at \(df_Q=0\), flow-line coupling, bifundamental vev, or singularity-deformation mixing & derive the flow-overlap product from the point-support metric and local wavefunctions \\
...
\((J_H,J_{\rm adj})=(3/4,2)\) & localized doublet/order-parameter data, adjoint/current data, and photon-orthogonal completion data & derive \(P_W\), \(P_Z\), \(P_\gamma\), hypercharge, charge lattice, and pole map in the same compact model \\
\bottomrule
\end{tabular}
\end{center}
...
consistency check.  A \(G_2\) route that supports the manuscript claim has to
...
interaction evidence; the quadratic kernel requires the two-point flow-line
...
\begin{equation}
  ...
  \qquad
  \mathsf G_Q\le0,\\
  \hbox{orientation sum lacks a source convention},
  \qquad

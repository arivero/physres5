...
\(\langle\cdot,\cdot\rangle_{u_{\rm EW}}\), and let \(a_\gamma\) be the unit
photon trace.  The massive neutral sample is
\begin{equation}
  \Pi_{\gamma^\perp}^{(T)}
  =
  ...
  a_J^{\gamma^\perp}
  =
  P_{a,J}\Pi_{\gamma^\perp}^{(T)}(W^3_T,B_T),
...
target is \(\Delta_Y=0\) in the selected convention.
...
samples the doublet Goldstone/order-parameter direction, and the neutral
insertion samples the photon-orthogonal current direction of

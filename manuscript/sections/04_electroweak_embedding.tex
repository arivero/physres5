...
The interval realization gives a concrete test of the same package.  With
\(u_{\rm int}\) denoting the interval source datum, the CHM boundary machinery
must fit into the theorem diagram
\begin{equation}
\begin{aligned}
u_{\rm int}
&\longmapsto
...
u_{\rm int}
&\longmapsto
\left(v,g,g',P_W,P_Z,P_\gamma,\Lambda_J\right)
\longmapsto
\frac{M_{W,\rm pole}^2}{M_{Z,\rm pole}^2}
=
\frac{x_+(3/4)}{x_+(2)}
...
\end{aligned}
\label{eq:o1-o10-source-kernel-diagram}
\end{equation}
Here \(K_T^{\rm DtN}\) is the transverse Dirichlet-to-Neumann kernel,
\(K_T^{\rm brane}\) denotes boundary mass and kinetic contributions, and
\(\widehat K_J\) is the normalized two-channel kernel after photon subtraction
and projection.  This diagram is a theorem target: CHM supplies the boundary
operators and photon-zero bookkeeping, while the equality above requires the
projection, normalization, and pole remainder to be derived from the same
source datum.

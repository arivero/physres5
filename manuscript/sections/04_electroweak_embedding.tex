...
\begin{equation}
  ...
  \label{eq:rho-tree}
\end{equation}
This is the electroweak reason the DeVries comparison should be a projective statement about the W/Z pair.  The broken-to-unbroken deformation scales the vector masses with \(v\) while preserving the coupling-space direction.
...
\begin{align}
  ...
  \frac{M_W^2}{M_Z^2}&=\frac{x_+(J_H)}{x_+(J_{\rm adj})}.
  \label{eq:assignment-outputs}
\end{align}
The theorem must also account for the negative branch in the same basis.  This prevents the vector ratio from being fitted by one argument and the scalar clue by another.
...
\end{equation}
which transforms as an adjoint object while being built from the doublet order parameter.  This current has the right qualitative structure for the ordered pair: doublet data enter the source, adjoint data enter the current.  Turning this observation into Eq.~\eqref{eq:assignment-outputs} is the electroweak calculation owed.

\paragraph*{Section status.}  The projective interpretation follows from the electroweak mass matrix.  The adjoint and doublet Casimirs are identified.  The ordered sampling rule in Eq.~\eqref{eq:Jassignment} remains the primary open calculation.

...
\(\Sigma_{ha,J}^{\rm int}\Sigma_{ah,J}^{\rm int}=J\).  The same interval
model must still derive the diagonal entry \(J\), the sign convention in the
DeVries block, and the ordered electroweak map.  Appendix~\ref{app:target-susyqm-origin}
records this as a theorem target.

...
\end{equation}
The first arrow is historical motivation until a world-sheet or boundary calculation produces the second expression.  The second arrow is algebraic and already fixed.  The Rivero/DeVries observation functions as a clue that selects a target determinant.
...
A related brane-scaling test adds an axis map to
Eq.~\eqref{eq:regge-intercept-extension}.  A source theory must state whether
the DeVries sector label maps to a putative rotating-brane angular momentum or
serves as an intercept label.  The shared source package is
\[
  \mathfrak R_j=
  (K_j,\alpha',P_{\rm surv},\mathcal B_{\rm ND},
  \mathcal Q,\mathcal T,\chi_{\rm spin}),
\]
where \(\chi_{\rm spin}\) is the extra axis map.  The current algebraic test is
summarized in Appendix~\ref{app:target-branch-scaling}: the positive branch is
bounded at large putative brane spin, while the negative magnitude grows
linearly under the mass reading.
...
The open-string route also clarifies the negative branch.  The two roots are eigenvalues of one two-channel system.  If the positive branch is associated with the vector pole quotient, the negative branch should correspond to the partner channel in the same boundary problem.  In an endpoint construction this partner may be a scalar boundary modulus, a Higgs-like order parameter, a brane separation mode, or an auxiliary channel eliminated in the low-energy action.  This makes the negative branch a demand on the model: the same determinant has to explain both eigenvalues.

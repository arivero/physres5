...
The rendered CHM convention audit gives concrete boundary data that can feed
this target.  In the scalar example, a boundary kinetic term at \(y=0\) has
...
\(\mathcal N_T^\partial\) be the neutral transverse boundary-trace space
spanned by \(W^3_T\) and \(B_T\), with CHM product
\(\langle\cdot,\cdot\rangle_{\rm CHM}\).  The projection data are
\begin{equation}
  P_{a,J}^{\gamma^\perp}:\mathcal N_T^\partial\to\gamma^\perp,
  \qquad
  P_\gamma:\mathcal N_T^\partial\to{\rm span}\{a_\gamma\},
  \qquad
  \langle a_J,a_\gamma\rangle_{\rm CHM}=0,
  \label{eq:chm-neutral-trace-space}
\end{equation}
where \(a_J=P_{a,J}^{\gamma^\perp}A_\mu^{T,\partial}\) and
\(a_\gamma=P_\gamma A_\mu^{T,\partial}\) have unit CHM norm.  The transverse
kernel is
\[
...
\end{equation}
This is the CHM current-entry theorem target.  The source equations audited for
this target are the CHM boundary kinetic equations, modified product, vector
...
\(P_{a,J}^{\gamma^\perp}\), \(P_\gamma\), \(K^{\rm ref}_{T,J}\),
...
\[
  ...
  K^{\rm ref}_{T,J},\quad \Lambda_{\rm CHM},\quad
  \mathcal R_{\rm pole}.
\]
Closure of the interval route requires these data to yield, in one convention,
\[
...
\begin{align}
  ...
  a_J^{\gamma^\perp}&=
  P_{a,J}^{\gamma^\perp}A_\mu^{T,\partial},
  \qquad
  a_\gamma=P_\gamma A_\mu^{T,\partial}.
...
\end{equation}
with one \(\Lambda_{\rm CHM}\), one product, and one pole map.
...
\(P_W\), \(P_Z\), \(K^{\rm ref}_{T,J}\), the source scale
